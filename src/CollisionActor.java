...
public class CollisionActor extends Actor
{
    ...
    public void setImage(MayflowerImage newImage)
    {
        ...
        for(int r = 0; r < pixelLocation.length; r++)
        {
            ...
            {
                if(img.getColorAt(r, c).getAlpha() == 0)
                {
                    pixelLocation[r][c] = 0;
                }
                else
        ...
        System.out.println(Arrays.deepToString(pixelLocation));
    ...
    public boolean checkPixelCollision(CollisionActor actor)
    {
        ...
        for(int r = finLeft; r <= finRight; r++)
        {
            for(int c = finTop; c <= finBottom; c++) {
                System.out.println(pixelLocation[r][c]);
                System.out.println(actor.getPixelLocation()[r][c]);
    ...
    public void setCurrentImage(MayflowerImage img)
    {
        this.img = img;
    }
